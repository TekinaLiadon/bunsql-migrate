...
export function cliEnv(
  options: { databaseUrl?: string; listDir?: string; extra?: Record<string, string> } = {},
): Record<string, string> {
  const env: Record<string, string> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "DATABASE_URL" || key === "MIGRATION_LIST_DIR" || value === undefined) {
      continue;
    }
    env[key] = value;
  }
  ...
  if (options.listDir !== undefined) {
    env["MIGRATION_LIST_DIR"] = options.listDir;
  }
...
export function withSqlite<T>(dbPath: string, run: (db: Database) => T): T {
...
export function readRowCount(dbPath: string, table = "migrations"): number {
  return withSqlite(dbPath, (db) => {
